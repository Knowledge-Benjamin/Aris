...
/**
 * Reads the authenticated user's own WhatsApp JID dynamically from the stored
 * Baileys credentials. Returns null if no account has been paired yet.
 */
export async function getSelfJid(): Promise<string | null> {
  try {
    ...
    const creds = typeof res.rows[0].value === "string"
